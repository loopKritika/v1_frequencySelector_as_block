...
function getButtonText(isOneTimeOrder, productId) {
    if (!isOneTimeOrder) {
        if (document.querySelector('.loop-upd') != null) {
            return (
                "Add To Cart"
            );
        } else {
...
function updatePriceInUI({ productId }) {
    ...
    selling_plan_groups.map((spg) => {
        if (Array.isArray(spg.selling_plans) && spg.selling_plans.length) {
            ...
            if (element) {
                if(compare_at_price > per_delivery_price) {
                    if ($(".loop-subscription-container").hasClass('loop-upd')) {
                      element.innerHTML = `<span class="m-original sd">${loopFormatMoney(compare_at_price, true)}</span>${loopFormatMoney(per_delivery_price, true)}`;
                    }
                } else {
    ...
    if (sellingPlan && sellingPlan.selling_plan_group_id) {
        const { selling_plan_group_id, per_delivery_price, compare_at_price } = sellingPlan;
        let element = document.querySelector(
            `#loop-price-${variant.id}-${selling_plan_group_id}`
        );
        if (element) {
            if(compare_at_price > per_delivery_price && document.querySelector('.loop-upd') != null) {
                element.innerHTML = `<span class="m-original ds">${loopFormatMoney(compare_at_price, true)}</span>${loopFormatMoney(per_delivery_price, true)}`;
            } else {
                element.innerHTML = loopFormatMoney(per_delivery_price, true);
            }
        }
...
function applyBundleDiscount(productId) {
        ...
        if (Array.isArray(spg.selling_plans) && spg.selling_plans.length) {
            ...
            if (element) {
                ...
                let totalDiscount = Math.round(
                    (((variant.price ?? compare_at_price) - val) /
                        (variant.price ?? compare_at_price)) *
                        100
                );
    ...
    if (loopOneTimePriceOriginal) {
        let val = variant.price ?? compare_at_price;
